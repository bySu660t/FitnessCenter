            ...
            if (clientdataGridView.CurrentRow.Index != -1)
            {
                ...
                if (trainersDataGridView.CurrentRow.Index != -1)
                {
                    ...
                    using (GymDBEntities db = new GymDBEntities())
                    {
                        ...
                        if (str != null)
                        {
                            db.Database.ExecuteSqlCommand($"update schedule set enddate=getdate() where trainer_id = {train_id} and customer_id = {cust_id} and enddate is null");
                            MessageBox.Show("Запись успешно отменена!", "Успешная операция", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            Clear(true);
                        }
                        else
                        {
                            MessageBox.Show("Записи не существует и занятия отменить невозможно!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                }
                else
